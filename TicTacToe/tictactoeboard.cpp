    ...
    : QWidget(parent)
{
    ...
    current_player_label->setText(QApplication::translate("player", "Player 1(X) starts the game."));
    tictacLayout->addWidget(current_player_label, 0, 1);
    ...
    tictacLayout->addWidget(button13, 1, 2);  // Add button3 spanning from row 1 to row 2 and column 0 to column 1
...
void TicTacToeBoard::handleButton(){
    ...
    emit buttonPressed(lastButtonClicked); // Send the pressed button to gameLogic to update stuff. Since it points to the button the gameLogic should be able to update the text of the box.
}
